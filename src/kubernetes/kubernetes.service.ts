...
import { K8S_SA_TOKEN_PATH, IS_PROD } from 'src/common/utils';
...
type KubernetesClientBase = Awaited<
  ReturnType<KubernetesService['getClientBase']>
>;
...
export class KubernetesService {
  ...
  async getClient(
    ...
    const bffClient = await this.getClientBase(
      { name: 'bff-sa', token: this.k8sConfig.bffSaToken, ip: user.ip },
      // 只支持管理集群，不支持指定其他集群
  ...
  private async getClientBase(user: K8sUser, options: ClientOptions = {}) {
    ...
    const rbacAuthorizationV1Api = kubeConfig.makeApiClient(
    ...
    const authenticationV1Api = kubeConfig.makeApiClient(
