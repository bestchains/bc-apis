...
export class Job {
   ...
   * @param {K8s.V1Job} body Job 对象
   * @param {CreateOptions} [options] 可选配置项
   */
  create(namespace: string, body: K8s.V1Job, options?: CreateOptions) {
    this.debug('[create]', body, options);
    const { pretty, dryRun, fieldManager, fieldValidation, headers } =
      options || {};
    return this.k8sApi.createNamespacedJob(
      namespace,
      body,
      pretty,
      dryRun,
  ...
  replace(
    ...
    const { pretty, dryRun, fieldManager, fieldValidation, headers } =
      options || {};
    return this.k8sApi.replaceNamespacedJob(
      name,
      namespace,
      body,
      pretty,
  ...
  /**
   * 部分更新指定的 Job (JSON Patch)
   *
   * @param {string} name Job 名称
   * @param {string} namespace 命名空间
   * @param {JsonPatchOp[]} body Job 对象
  ...
  patch(
    ...
    const { pretty, dryRun, fieldManager, fieldValidation, force, headers } =
      options || {};
    return this.k8sApi.patchNamespacedJob(
      name,
      namespace,
      body,
      pretty,
  ...
  /**
   * 部分更新指定的 Job (Merge Patch)
   *
   * @param {string} name Job 名称
   * @param {string} namespace 命名空间
   * @param {object} body Job 对象
   * @param {PatchOptions} [options] 可选配置项
   */
  patchMerge(
    name: string,
  ...
  /**
   * 部分更新指定的 Job (Strategic Merge Patch)
   *
   * @param {string} name Job 名称
   * @param {string} namespace 命名空间
   * @param {object} body Job 对象
  ...
  delete(name: string, namespace: string, options?: DeleteOptions) {
    ...
    return this.k8sApi.deleteNamespacedJob(
      name,
      namespace,
      pretty,
      dryRun,
  ...
  deleteCollection(
    ...
    return this.k8sApi.deleteCollectionNamespacedJob(
      namespace,
      pretty,
      _continue,
      dryRun,
  ...
  read(name: string, namespace: string, options?: ReadOptions) {
    ...
    return this.k8sApi.readNamespacedJob(name, namespace, pretty, { headers });
  }
  ...
  list(namespace: string, options?: ListOptions) {
    ...
    return this.k8sApi.listNamespacedJob(
      namespace,
      pretty,
      allowWatchBookmarks,
      _continue,
  ...
  listAll(options?: ListOptions) {
    ...
    return this.k8sApi.listJobForAllNamespaces(
      allowWatchBookmarks,
      _continue,
      fieldSelector,
      labelSelector,
