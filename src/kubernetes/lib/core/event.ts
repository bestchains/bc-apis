...
export class Event {
   ...
   * @param {K8s.CoreV1Event} body Event 对象
   * @param {CreateOptions} [options] 可选配置项
   */
  create(namespace: string, body: K8s.CoreV1Event, options?: CreateOptions) {
    this.debug('[create]', body, options);
    const { pretty, dryRun, fieldManager, fieldValidation, headers } =
      options || {};
    return this.k8sApi.createNamespacedEvent(
      namespace,
      body,
      pretty,
      dryRun,
  ...
  replace(
    name: string,
    namespace: string,
    body: K8s.CoreV1Event,
    options?: CreateOptions,
  ) {
    this.debug(`[replace] ns:${namespace} => ${name}`, body, options);
    const { pretty, dryRun, fieldManager, fieldValidation, headers } =
      options || {};
    return this.k8sApi.replaceNamespacedEvent(
      name,
      namespace,
      body,
      pretty,
  ...
  /**
   * 部分更新指定的 Event (JSON Patch)
   *
   * @param {string} name Event 名称
   * @param {string} namespace 命名空间
   * @param {JsonPatchOp[]} body Event 对象
  ...
  patch(
    ...
    const { pretty, dryRun, fieldManager, fieldValidation, force, headers } =
      options || {};
    return this.k8sApi.patchNamespacedEvent(
      name,
      namespace,
      body,
      pretty,
  ...
  /**
   * 部分更新指定的 Event (Merge Patch)
   *
   * @param {string} name Event 名称
   * @param {string} namespace 命名空间
   * @param {object} body Event 对象
   * @param {PatchOptions} [options] 可选配置项
   */
  patchMerge(
    name: string,
  ...
  /**
   * 部分更新指定的 Event (Strategic Merge Patch)
   *
   * @param {string} name Event 名称
   * @param {string} namespace 命名空间
   * @param {object} body Event 对象
  ...
  delete(name: string, namespace: string, options?: DeleteOptions) {
    ...
    return this.k8sApi.deleteNamespacedEvent(
      name,
      namespace,
      pretty,
      dryRun,
  ...
  deleteCollection(
    ...
    return this.k8sApi.deleteCollectionNamespacedEvent(
      namespace,
      pretty,
      _continue,
      dryRun,
  ...
  /**
   * 根据名称获取 Event 详情 (指定 namespace 下)
   *
   * @param {string} namespace 命名空间
  ...
  read(name: string, namespace: string, options?: ReadOptions) {
    ...
    return this.k8sApi.readNamespacedEvent(name, namespace, pretty, { headers });
  }
  ...
  list(namespace: string, options?: ListOptions) {
    ...
    return this.k8sApi.listNamespacedEvent(
      namespace,
      pretty,
      allowWatchBookmarks,
      _continue,
  ...
  listAll(options?: ListOptions) {
    ...
    return this.k8sApi.listEventForAllNamespaces(
      allowWatchBookmarks,
      _continue,
      fieldSelector,
      labelSelector,
