...
export class LimitRange {
   ...
   * @param {K8s.V1LimitRange} body LimitRange 对象
   * @param {CreateOptions} [options] 可选配置项
   */
  create(namespace: string, body: K8s.V1LimitRange, options?: CreateOptions) {
    this.debug('[create]', body, options);
    const { pretty, dryRun, fieldManager, fieldValidation, headers } =
      options || {};
    return this.k8sApi.createNamespacedLimitRange(
      namespace,
      body,
      pretty,
      dryRun,
   ...
   * 替换指定的 LimitRange
   *
   * @param {string} name LimitRange 名称
   * @param {string} namespace 命名空间
   * @param {K8s.V1LimitRange} body LimitRange 对象
   * @param {CreateOptions} [options] 可选配置项
   */
  replace(
    name: string,
    namespace: string,
    body: K8s.V1LimitRange,
    options?: CreateOptions,
  ) {
    this.debug(`[replace] ns:${namespace} => ${name}`, body, options);
    const { pretty, dryRun, fieldManager, fieldValidation, headers } =
      options || {};
    return this.k8sApi.replaceNamespacedLimitRange(
      name,
      namespace,
      body,
      pretty,
  ...
  /**
   * 部分更新指定的 LimitRange (JSON Patch)
   *
   * @param {string} name LimitRange 名称
   * @param {string} namespace 命名空间
   * @param {JsonPatchOp[]} body LimitRange 对象
  ...
  patch(
    ...
    const { pretty, dryRun, fieldManager, fieldValidation, force, headers } =
      options || {};
    return this.k8sApi.patchNamespacedLimitRange(
      name,
      namespace,
      body,
      pretty,
  ...
  /**
   * 部分更新指定的 LimitRange (Merge Patch)
   *
   * @param {string} name LimitRange 名称
   * @param {string} namespace 命名空间
   * @param {object} body LimitRange 对象
   * @param {PatchOptions} [options] 可选配置项
   */
  patchMerge(
    name: string,
  ...
  /**
   * 部分更新指定的 LimitRange (Strategic Merge Patch)
   *
   * @param {string} name LimitRange 名称
   * @param {string} namespace 命名空间
   * @param {object} body LimitRange 对象
  ...
  delete(name: string, namespace: string, options?: DeleteOptions) {
    ...
    return this.k8sApi.deleteNamespacedLimitRange(
      name,
      namespace,
      pretty,
      dryRun,
  ...
  deleteCollection(
    ...
    return this.k8sApi.deleteCollectionNamespacedLimitRange(
      namespace,
      pretty,
      _continue,
      dryRun,
  ...
  /**
   * 根据名称获取 LimitRange 详情 (指定 namespace 下)
   *
   * @param {string} namespace 命名空间
  ...
  read(name: string, namespace: string, options?: ReadOptions) {
    ...
    return this.k8sApi.readNamespacedLimitRange(name, namespace, pretty, { headers });
  }
  ...
  list(namespace: string, options?: ListOptions) {
    ...
    return this.k8sApi.listNamespacedLimitRange(
      namespace,
      pretty,
      allowWatchBookmarks,
      _continue,
  ...
  /**
   * 列取 LimitRange 列表 (所有 namespace 下)
   *
   * @param {ListOptions} [options] 可选配置项
   */
  listAll(options?: ListOptions) {
    ...
    return this.k8sApi.listLimitRangeForAllNamespaces(
      allowWatchBookmarks,
      _continue,
      fieldSelector,
      labelSelector,
