...
export class DaemonSet {
   ...
   * @param {K8s.V1DaemonSet} body DaemonSet 对象
   * @param {CreateOptions} [options] 可选配置项
   */
  create(namespace: string, body: K8s.V1DaemonSet, options?: CreateOptions) {
    this.debug('[create]', body, options);
    const { pretty, dryRun, fieldManager, fieldValidation, headers } =
      options || {};
    return this.k8sApi.createNamespacedDaemonSet(
      namespace,
      body,
      pretty,
      dryRun,
   ...
   * 替换指定的 DaemonSet
   *
   * @param {string} name DaemonSet 名称
   * @param {string} namespace 命名空间
   * @param {K8s.V1DaemonSet} body DaemonSet 对象
   * @param {CreateOptions} [options] 可选配置项
   */
  replace(
    ...
    const { pretty, dryRun, fieldManager, fieldValidation, headers } =
      options || {};
    return this.k8sApi.replaceNamespacedDaemonSet(
      name,
      namespace,
      body,
      pretty,
  ...
  /**
   * 部分更新指定的 DaemonSet (JSON Patch)
   *
   * @param {string} name DaemonSet 名称
   * @param {string} namespace 命名空间
   * @param {JsonPatchOp[]} body DaemonSet 对象
  ...
  patch(
    ...
    const { pretty, dryRun, fieldManager, fieldValidation, force, headers } =
      options || {};
    return this.k8sApi.patchNamespacedDaemonSet(
      name,
      namespace,
      body,
      pretty,
  ...
  /**
   * 部分更新指定的 DaemonSet (Merge Patch)
   *
   * @param {string} name DaemonSet 名称
   * @param {string} namespace 命名空间
   * @param {object} body DaemonSet 对象
   * @param {PatchOptions} [options] 可选配置项
   */
  patchMerge(
    name: string,
  ...
  /**
   * 部分更新指定的 DaemonSet (Strategic Merge Patch)
   *
   * @param {string} name DaemonSet 名称
   * @param {string} namespace 命名空间
   * @param {object} body DaemonSet 对象
  ...
  delete(name: string, namespace: string, options?: DeleteOptions) {
    ...
    return this.k8sApi.deleteNamespacedDaemonSet(
      name,
      namespace,
      pretty,
      dryRun,
  ...
  deleteCollection(
    ...
    return this.k8sApi.deleteCollectionNamespacedDaemonSet(
      namespace,
      pretty,
      _continue,
      dryRun,
  ...
  /**
   * 根据名称获取 DaemonSet 详情 (指定 namespace 下)
   *
   * @param {string} namespace 命名空间
  ...
  list(namespace: string, options?: ListOptions) {
    ...
    return this.k8sApi.listNamespacedDaemonSet(
      namespace,
      pretty,
      allowWatchBookmarks,
      _continue,
  ...
  /**
   * 列取 DaemonSet 列表 (所有 namespace 下)
   *
   * @param {ListOptions} [options] 可选配置项
   */
  listAll(options?: ListOptions) {
    ...
    return this.k8sApi.listDaemonSetForAllNamespaces(
      allowWatchBookmarks,
      _continue,
      fieldSelector,
      labelSelector,
