...
export const CHAINCODE_BUILD_CRD_SPEC = {
  kind: 'ChaincodeBuild',
...
const { kind, group, version, plural } = CHAINCODE_BUILD_CRD_SPEC;
const DEFAULT_BODY = {
  kind,
...
export class ChaincodeBuild {
   ...
   * 创建 ChaincodeBuild
   *
   * @param {CRD.ChaincodeBuild} body ChaincodeBuild 对象
   * @param {CreateOptions} [options] 可选配置项
   */
  create(body: CRD.ChaincodeBuild, options?: CreateOptions) {
    ...
    return this.k8sApi.createClusterCustomObject(
      ...
      body: CRD.ChaincodeBuild;
    }>;
  }

  /**
   * 替换指定的 ChaincodeBuild
   *
   * @param {string} name ChaincodeBuild 名称
   * @param {CRD.ChaincodeBuild} body ChaincodeBuild 对象
   * @param {CreateOptions} [options] 可选配置项
   */
  replace(name: string, body: CRD.ChaincodeBuild, options?: CreateOptions) {
  ...
  /**
   * 部分更新指定的 ChaincodeBuild (JSON Patch)
   *
   * @param {string} name ChaincodeBuild 名称
   * @param {JsonPatchOp[]} body ChaincodeBuild patch json 对象
  ...
  /**
   * 部分更新指定的 ChaincodeBuild (Merge Patch)
   *
   * @param {string} name ChaincodeBuild 名称
   * @param {object} body ChaincodeBuild 对象
   * @param {PatchOptions} [options] 可选配置项
   */
  ...
  /**
   * 部分更新指定的 ChaincodeBuild (Strategic Merge Patch)
   *
   * @param {string} name ChaincodeBuild 名称
   * @param {object} body ChaincodeBuild 对象
   * @param {PatchOptions} [options] 可选配置项
